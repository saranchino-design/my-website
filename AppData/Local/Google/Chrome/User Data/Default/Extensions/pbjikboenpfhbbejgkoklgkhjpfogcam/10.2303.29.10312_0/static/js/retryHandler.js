...
var deprecationMessage = namespace.i18n.getMessage("deprecationMessage");
var uninstallButton = namespace.i18n.getMessage("uninstallButton");
...
window.addEventListener("DOMContentLoaded", function () {
    ...
    // Update Deprecation Message with correct language, if available
    if (document.getElementById("deprecationMessage") && deprecationMessage) {
        document.getElementById("deprecationMessage").innerHTML = deprecationMessage;
    }

    // Update Uninstall Button with correct language, if available
    if (document.getElementById("uninstallButton") && uninstallButton) {
        document.getElementById("uninstallButton").textContent = uninstallButton;
    }

    // Add uninstallSelf function to Uninstall Button, if available
    if (document.getElementById("uninstallButton")) {
        document.getElementById("uninstallButton").addEventListener("click", function () {
            reportLoadEvent("Uninstall")

            if (namespace && namespace.management) {
                namespace.management.uninstallSelf({showConfirmDialog: true});
            }
        });
    }
});
