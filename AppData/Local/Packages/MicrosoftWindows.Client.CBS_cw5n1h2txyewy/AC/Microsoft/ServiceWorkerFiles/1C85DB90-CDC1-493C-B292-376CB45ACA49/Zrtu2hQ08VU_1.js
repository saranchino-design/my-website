...
(function (WSBServiceWorker) {
    ...
    WSBServiceWorker.asTrieCacheName = "asTrieCache";
    ...
    async function getResponseFromCache(request, isResourceRequest, isThumbRequest, isASTrieRequest) {
        let response;
        ...
        else if (config.enableASTrieInGenericCache && isASTrieRequest) {
            response = await asTrieCache.match(request, WSBCacheMatchOptions);
            if (response) {
                return response;
            }
        }
    ...
    async function getResponseFromServer(request, isInit, isResourceRequest, isThumbRequest, isASTrieResourceRequest) {
        let response;
        try {
            if (isInit) {
            ...
            if (response === null || response === void 0 ? void 0 : response.ok) {
                ...
                else if (config.enableASTrieInGenericCache && isASTrieResourceRequest) {
                    if (asTrieCache) {
                        asTrieCache.put(request, responseClone);
                    }
                }
            }
            else {
    ...
    async function prepareCaches() {
        ...
        if (config.enableASTrieInGenericCache && !asTrieCache) {
            asTrieCache = await caches.open(WSBServiceWorker.asTrieCacheName);
        }
    ...
    async function onFetchWithGenericCache(request) {
        var _a;
        if (config.fixInvalidMUID && sj_cook) {
            sj_cook.clear("MUID", "/");
        }
        ...
        const isASTrieResource = isASTrieResourceRequest(requestUrlStr);
        ;
        ...
        await prepareCaches();
        if (isResourceRequest) {
            response = await getResponseFromCache(request, isResourceRequest, isThumbRequest, isASTrieResource);
            if (response) {
        ...
        response = await getResponseFromServer(request, isInit, isResourceRequest, isThumbRequest, isASTrieResource);
        if (config.fixEmptyResponseError && !response) {
            response = Response.error();
        }
        return response;
    }
    WSBServiceWorker.onFetchWithGenericCache = onFetchWithGenericCache;
    async function onFetchWithGenericCacheWithTimeout(request) {
        let timerId;
        const timeoutPromise = new Promise((resolve, reject) => timerId = setTimeout(() => {
            let response = new Response("Request timed out", { status: 504, statusText: "Gateway Timeout" });
            bundleFetchImpression === null || bundleFetchImpression === void 0 ? void 0 : bundleFetchImpression.logError2("Fetch timeout", request === null || request === void 0 ? void 0 : request.url);
            resolve(response);
        }, config.swFetchTimeoutInMS));
        try {
            const response = await Promise.race([
                onFetchWithGenericCache(request),
                timeoutPromise
            ]);
            if (timerId) {
            ...
            bundleFetchImpression === null || bundleFetchImpression === void 0 ? void 0 : bundleFetchImpression.logError2("onFetchWithGenericCacheWithTimeout", request === null || request === void 0 ? void 0 : request.url, err);
            return Response.error();
        }
    }
    WSBServiceWorker.onFetchWithGenericCacheWithTimeout = onFetchWithGenericCacheWithTimeout;
    /**
    ...
    function isThumbnailRequest(url) {
        ...
        if (url) {
            // Chinese image host name could be s.cn.bing.net or ts1.tc.mm.bing.net
            // other regions is th.bing.com
            return url.includes("th.bing.com") || url.includes("s.cn.bing.net") || url.includes("ts1.tc.mm.bing.net");
        }
        return false;
    }
    ...
    function isASTrieResourceRequest(url) {
        if (config.enableASTrieInGenericCache && url) {
            return url.includes("/commonautosuggest/local-trie/");
        }
        return false;
    }
    ...
    swSelf.addEventListener("fetch", (ev) => {
        if (config.enableGenericCacheInAllWebView) {
            ev.respondWith(initializeServiceWorker().then(() => {
                if (config.swFetchWithTimeout) {
                    return onFetchWithGenericCacheWithTimeout(ev.request).catch((reason) => {
                        pickLogger(bundleFetchImpression, "onFetch", reason);
                        return null;
                    });
                }
                else {
                    return onFetchWithGenericCache(ev.request).catch((reason) => {
                        pickLogger(bundleFetchImpression, "onFetch", reason);
                        return null;
                    });
                }
